#!/usr/bin/env python3
"""Minimal evolution-stage sprite staging prep.

GenerateImage 256px → outline-protected flood → fit to game evolution REF_BBOX.
"""
...
# Content bboxes from assets/sprites/evolution/*.png
...
def parse_stage(path: Path) -> str:
    m = re.search(r"deepsea-evo-(egg|baby|child|teen|ghost)-", path.name)
    if not m:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evolution sprite staging prep")
    parser.add_argument("input", type=Path, help="Generated PNG")
    parser.add_argument("output", type=Path, nargs="?", help="Output path")
    parser.add_argument(
        "--allow-white-loss",
        action="store_true",
        help="Do not exit on >30%% near-white loss (ghost / pale sprites)",
    )
