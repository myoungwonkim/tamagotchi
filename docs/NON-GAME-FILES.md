...
> 감사 기준: 2026-07-31. 참조 여부는 저장소 전체 grep으로 검증했습니다.

## 정리된 위치 (2026-07-31)

미적용 파일은 **삭제하지 않고** 아래로 모았습니다.
...
- `docs/previews/*.html` — 시안 95개(git 추적됨; `.cursorignore`의 `docs/previews/**`로 에이전트 기본 검색에서는 숨겨짐).
- 프리뷰 폴더: `docs/previews/header-icons-preview/`, `speaker-icon-preview/`, `mermaid-side-preview/`, `vent-theme-preview/`.
- `docs/previews/index.html`(시안 갤러리), `docs/previews/c.html`.
- 갤러리 링크는 모두 같은 폴더 내 상대경로라 이동 후에도 그대로 동작합니다.
- 이 중 23개는 갤러리에서 링크, 나머지 ~70개는 어디서도 링크 안 됨.
...
### D. 기타
- `docs/*.md` — 프로젝트 문서 19개(런타임 아님, 살아있는 참고 문서 — 이 파일 포함).
- `docs/legacy/privacy.html`, `docs/legacy/terms-of-service.html` — 오래된 리다이렉트 스텁(`docs/` 미배포라 무의미).
...
| 그룹 | 삭제 가능? | 사유 |
|---|---|---|
| `docs/previews/*.html` 중 갤러리 미링크(~70) | **예** | 저장소 전용·미배포·런타임/CI 참조 없음. |
| `docs/previews/*.html` 중 갤러리 링크(23) + `docs/previews/index.html` | **주의** | 공개 사이트엔 없지만 개별 삭제 시 로컬 갤러리 링크 깨짐. 갤러리 통째 제거 시 안전. 일부는 `PNG-SPRITE-GUIDE.md`에서도 링크. |
| 프리뷰 폴더 4종 | **주의** | 갤러리 링크; `speaker-icon-`·`mermaid-side-`는 추적 스크립트로 재생성 가능. |
| `docs/previews/ui-icon-redesign-preview.html`, `scripts/prepare_ui_redesign.py` | **예** | 참조 0. |
...
| `docs/legacy/**` | **예** | 미배포 스텁; 정본은 루트 페이지. |
...
1. `docs/previews/index.html` 갤러리는 공개 미배포 — 내부 아카이브로 유지할지, 링크된 23개 프리뷰와 함께 제거할지 결정.
