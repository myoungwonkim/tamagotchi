...
const DEEPSEA_ENCYCLOPEDIA_DESCRIPTIONS = {
  ...
  sparkle:
    "{name}는 둥근 몸통과 커다란 눈으로 바닥 모래 속에 숨어 있다가, 먹이가 지나가면 촉수를 쫙 펼쳐요. 귀처럼 생긴 지느러미는 느리게 살랑거리고, 몸의 청록 점들은 밤마다 한 박자 늦게 반짝입니다. 유영 속도는 매우 느린데, 본인은 '심사숙고 중'이라고 주장해요. 장난감 인형 같다는 말을 들으면 기분이 좋아져 더 밝게 빛납니다.",
  standard:
...
const MERMAID_ENCYCLOPEDIA_DESCRIPTIONS = {
  golden:
    "{name}은 금빛 비늘과 창을 든 심해 어인 호위예요. 지느러미 방패는 물살을 가르며 빛나고, 뿔볏은 멀리서도 '여기 지킨다'는 표식처럼 보입니다. 금테 비늘이 떨어지면 주변 물고기가 행운이라 부르지만, {name}은 분실물이라고 고집합니다. 싸우지 않는 날에도 창을 세워 두면 기분이 좋아져요.",
  fluffy:
    "{name}은 등에 달린 거대한 소라 껍데기 때문에 헤엄치기보다 웅크리는 시간이 더 길어요. 뒤를 돌아보며 지나가는 이바귀를 확인하는 습관이 있어요. 껍데기가 무거워도 절대 벗지 않습니다.",
  sparkle:
  ...
  scruffy:
    "{name}은 오래된 심해 두루마리에서 걸어 나온 인어예요. 사람 얼굴에 팔다리까지 달렸지만 몸은 청록 비늘로 덮여 있어, 헤엄도 치고 바닥도 성큼성큼 걷습니다. 물살이 잔잔하면 두 손을 모으고 옛이야기를 시작하는데, 대부분 '내가 원래 전설이었어'로 끝나요. 진위는 아무도 확인해 주지 않지만, {name}은 개의치 않습니다.",
  grumpy:
